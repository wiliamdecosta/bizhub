 ...
 */
class Sortir_bizhub_detail_controller
{
    ...
    function read()
    {
        ...
        $sidx = getVarClean('sidx', 'str', 'sortir_bizhub_det_id');
        $sord = getVarClean('sord', 'str', 'desc');


        $data = array('rows' => array(), 'page' => 1, 'records' => 0, 'total' => 1, 'success' => false, 'message' => '');

        $sortir_bizhub_id = getVarClean('sortir_bizhub_id', 'int', 0);

        try {

            $ci = &get_instance();
            $ci->load->model('agripro/sortir_bizhub_detail');
            $table = $ci->sortir_bizhub_detail;
            ...
            // Filter Table
            $req_param['where'] = array("sort_det.sortir_bizhub_id = $sortir_bizhub_id ");
